...
def mcts(queue, iteration=500):
    ...
    for i in range(iteration):
        path = []  # 截止到当前节点的搜索路径
        node = root
        if len(path) + len(node.child) >= len(vacuity):   #判断该节点是否被完全拓展   len(vacuity)表示没有被访问的节点的个数  vacuity元素：元组类型
            node = selection(node, root.total, path)   #只有该节点被完全拓展才会Selection，才会在孩子节点中选择ucb1最大的一个
